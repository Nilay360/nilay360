          ...
          {BELIEFS.map((line, i) => (
            <Reveal key={line} delay={i * 0.09} y={18}>
              <div style={{ display: "flex", alignItems: "baseline", gap: "clamp(14px, 2vw, 24px)" }}>
                ...
                <p style={{ margin: 0, fontFamily: "'Cal Sans', Georgia, serif", fontWeight: 300, fontSize: "clamp(20px, 2.6vw, 40px)", lineHeight: 1.34, letterSpacing: "-0.016em", color: i === BELIEFS.length - 1 ? "#FFFFFF" : "rgba(255,255,255,0.78)" }}>{line}</p>
              </div>
            </Reveal>
          ))}
        </div>
