...
export function PropertyCard({
  ...
  const priceLabel =
    listing_type === "rent" ? `${formatPrice(price)}/mo` : formatPrice(price);
  ...
  return (
    <Link href={`/property/${slug}`} className={cn("group block", className)}>
      ...
      >
        ...
        <div
          className="relative w-full bg-[#121519] overflow-hidden"
        ...
        >
          {images[0] && !imgError ? (
            ...
            <div className="w-full h-full flex items-center justify-center bg-gradient-to-br from-[#0B0D10] to-[#121519]">
              <span className="text-[#2BA8E0] text-[13px] font-medium tracking-widest opacity-60">
                Nilay 360
              </span>
            </div>
          )}
        ...
        <div className="p-3 sm:p-4">
          {/* Price */}
          <div className="mb-2">
            <span className="font-display text-lg sm:text-xl font-semibold text-[#000000]">
              {priceLabel}
            </span>
          </div>

          {/* Title */}
          <h3 className="text-xs sm:text-sm font-medium text-[#000000] mb-1 line-clamp-2">
            {title}
          </h3>

          {/* Location */}
          <p className="text-xs text-[#6B7C72] mb-2 sm:mb-3 flex items-center gap-1">
            <svg
              width="10"
              height="10"
              viewBox="0 0 24 24"
          ...
          <div className="flex flex-wrap items-center gap-2 sm:gap-3 pt-2 sm:pt-3 border-t border-[rgba(27,67,50,0.08)]">
            {bedrooms !== undefined && (
              <div className="flex items-center gap-1 text-xs text-[#6B7C72]">
                <svg
                  width="12"
                  height="12"
                  viewBox="0 0 24 24"
                ...
                <strong className="text-[#000000] font-medium">
                  {bedrooms}
                </strong>{" "}
                <span className="hidden sm:inline">
                  {COMMERCIAL_CATEGORIES.includes(type) ? "Rooms" : "BHK"}
                </span>
              </div>
            )}
            {bathrooms !== undefined && (
              <div className="flex items-center gap-1 text-xs text-[#6B7C72]">
                ...
                <span className="hidden sm:inline">
                  {COMMERCIAL_CATEGORIES.includes(type) ? "Wash" : "Bath"}
                </span>
              </div>
            )}
            <div className="flex items-center gap-1 text-xs text-[#6B7C72] ml-auto">
              <svg
                width="12"
                height="12"
                viewBox="0 0 24 24"
