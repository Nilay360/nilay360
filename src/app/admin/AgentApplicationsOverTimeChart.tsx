...
// "Agent Applications Over Time" — agent_profiles.created_at (confirmed
// live; agent_profiles has no submitted_at column), last 12 months. Bar
// chart, same pairing convention as the buyer dashboard's
// InquiriesReceivedChart next to a line chart — reads as visually
// distinct from the line chart beside it rather than three
// identical-looking line charts.
...
export default function AgentApplicationsOverTimeChart() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      if (error) console.error("AgentApplicationsOverTimeChart — agent_profiles query error:", error);
  ...
  return (
    <ChartCard title="Agent Applications Over Time" value={data ? total : undefined} loading={!data}>
      <div style={{ width: "100%", height: 220 }}>
        <ResponsiveContainer>
          <BarChart data={data ?? []} margin={{ top: 4, right: 8, left: -20, bottom: 0 }}>
            ...
            <Bar dataKey="count" name="Applications" fill="#4ADE80" radius={[4, 4, 0, 0]} maxBarSize={28} />
          </BarChart>
        </ResponsiveContainer>
      </div>
    </ChartCard>
