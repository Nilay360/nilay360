...
const FONTS = "https://fonts.googleapis.com/css2?family=Cormorant+Garamond:ital,wght@0,300;0,400;0,500;0,600;0,700;1,300;1,400;1,600&family=DM+Sans:opsz,wght@9..40,300;9..40,400;9..40,500;9..40,600&display=swap";
  ...
  return (
    <nav className="sc-nav" style={{ position:"fixed", top:0, left:0, right:0, zIndex:200, height:60, display:"flex", alignItems:"center", justifyContent:"space-between", padding:"0 40px", background:"rgba(0,0,0,0.96)", backdropFilter:"blur(20px)", borderBottom:"0.5px solid rgba(43,168,224,0.15)" }}>
      ...
      <div className="sc-nav-actions" style={{ display:"flex", alignItems:"center", gap:10 }}>
        ...
        <a href="/post-property" style={{ padding:"7px 20px", borderRadius:7, background:"#2BA8E0", color:"#FFFFFF", fontSize:13, fontWeight:700 }}>Sell or Rent Property</a>
      </div>
    </nav>
  );
}
    ...
    <>
      ...
      <section className="sc-hero" style={{ background:"#050810", position:"relative", overflow:"hidden", padding:"140px 56px 72px" }}>
        ...
        <div style={{ position:"relative", maxWidth:1280, margin:"0 auto" }}>
          {badge && (
            <div style={{ display:"inline-flex", alignItems:"center", gap:8, background:"rgba(43,168,224,0.1)", border:"0.5px solid rgba(43,168,224,0.3)", borderRadius:99, padding:"6px 16px", marginBottom:22 }}>
              <span style={{ width:6, height:6, borderRadius:"50%", background:"#2BA8E0", display:"inline-block" }} />
              <span style={{ fontSize:11, color:"#2BA8E0", fontWeight:600, letterSpacing:"0.12em", textTransform:"uppercase" }}>{badge}</span>
            </div>
          )}
          {eyebrow && <div style={{ fontSize:11, color:"#2BA8E0", fontWeight:700, letterSpacing:"0.2em", textTransform:"uppercase", marginBottom:14 }}>{eyebrow}</div>}
          <h1 style={{ fontFamily:"'Cormorant Garamond',Georgia,serif", fontSize:"clamp(40px,5vw,60px)", fontWeight:300, color:"#fff", lineHeight:1.05, letterSpacing:"-0.01em" }}>
            {title}
            {italic && <em style={{ display:"block", color:"#3DBEF5", fontStyle:"italic", fontWeight:400 }}>{italic}</em>}
          </h1>
        ...
        <div style={{ maxWidth:1280, margin:"0 auto" }}>
              ...
              {bullets.map(b => (
                <div key={b.t} style={{ border:"1px solid rgba(255,255,255,0.08)", borderRadius:14, padding:28, background:"#0B0D10" }}>
                  <div style={{ width:40, height:40, borderRadius:10, background:"rgba(43,168,224,0.08)", display:"flex", alignItems:"center", justifyContent:"center", marginBottom:14 }}>
                    <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="#2BA8E0" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"><path d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>
                  </div>
                  <h3 style={{ fontFamily:"'Cormorant Garamond',Georgia,serif", fontSize:20, fontWeight:700, color:"#FFFFFF", marginBottom:8 }}>{b.t}</h3>
          ...
          <div className="sc-cta-banner" style={{ marginTop:48, padding:"36px 40px", background:"linear-gradient(135deg, #0B0D10 0%, #121519 100%)", borderRadius:16, display:"flex", justifyContent:"space-between", alignItems:"center", flexWrap:"wrap", gap:20 }}>
            ...
            <div style={{ display:"flex", gap:12, flexWrap:"wrap" }}>
              <a href="/properties" style={{ padding:"13px 28px", background:"#2BA8E0", borderRadius:8, color:"#FFFFFF", fontSize:14, fontWeight:700 }}>Browse Properties</a>
