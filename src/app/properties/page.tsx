    ...
    >
      <div style={{ position: "relative", height: "220px", overflow: "hidden", background: "#0a150f" }}>
        ...
        <div style={{ position: "absolute", top: "12px", left: "12px", display: "flex", gap: "6px" }}>
          <span style={{ padding: "4px 10px", borderRadius: "100px", fontSize: "10px", fontWeight: 700, letterSpacing: "0.1em", textTransform: "uppercase", background: property.listing_type === "rent" ? "rgba(45,106,79,0.9)" : "rgba(201,168,76,0.9)", color: property.listing_type === "rent" ? "#0A1526" : "#020C1C", backdropFilter: "blur(8px)" }}>
            {property.listing_type === "rent" ? "For Rent" : "For Sale"}
          </span>
          {property.is_featured && (
            <span style={{ padding: "4px 10px", borderRadius: "100px", fontSize: "10px", fontWeight: 700, letterSpacing: "0.1em", textTransform: "uppercase", background: "rgba(13,43,31,0.85)", color: "#10C4C3", border: "1px solid rgba(201,168,76,0.4)", backdropFilter: "blur(8px)" }}>Premium</span>
          )}
        </div>
        <button onClick={e => { e.preventDefault(); e.stopPropagation(); onToggleSave(property.id, { title: property.title, city: property.city, price: property.price, property_type: property.type, image: property.images?.[0] }); }} style={{ position: "absolute", top: "12px", right: "12px", width: "34px", height: "34px", borderRadius: "50%", background: "rgba(13,43,31,0.75)", border: "1px solid rgba(245,242,236,0.15)", display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer", backdropFilter: "blur(8px)" }}>
          <svg width="16" height="16" viewBox="0 0 24 24" fill={saved ? "#10C4C3" : "none"} stroke={saved ? "#10C4C3" : "rgba(245,242,236,0.6)"} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
            <path d="M20.84 4.61a5.5 5.5 0 0 0-7.78 0L12 5.67l-1.06-1.06a5.5 5.5 0 0 0-7.78 7.78l1.06 1.06L12 21.23l7.78-7.78 1.06-1.06a5.5 5.5 0 0 0 0-7.78z" />
          </svg>
        </button>
        <button onClick={handleCompare} title={!comparing && isFull ? "Comparison is full (max 3)" : comparing ? "Remove from comparison" : "Add to comparison"} style={{ position: "absolute", top: "12px", right: "54px", height: "34px", padding: "0 11px", borderRadius: "100px", background: comparing ? "#10C4C3" : "rgba(13,43,31,0.75)", border: comparing ? "none" : "1px solid rgba(245,242,236,0.15)", display: "flex", alignItems: "center", gap: "5px", cursor: !comparing && isFull ? "not-allowed" : "pointer", opacity: !comparing && isFull ? 0.5 : 1, backdropFilter: "blur(8px)", fontFamily: "'Cal Sans', sans-serif", fontSize: "10px", fontWeight: 700, letterSpacing: "0.06em", textTransform: "uppercase", color: comparing ? "#020C1C" : "rgba(245,242,236,0.8)" }}>
        ...
        <div style={{ position: "absolute", bottom: 0, left: 0, right: 0, background: "linear-gradient(transparent, rgba(0,0,0,0.7))", padding: "32px 16px 14px" }}>
          <span style={{ fontFamily: "'Cal Sans', Georgia, serif", fontSize: "22px", fontWeight: 600, color: "#10C4C3" }}>
      ...
      <div style={{ padding: "18px 20px 20px" }}>
        <h3 style={{ fontFamily: "'Cal Sans', Georgia, serif", fontSize: "17px", fontWeight: 600, color: "#020C1C", lineHeight: 1.3, marginBottom: "8px", display: "-webkit-box", WebkitLineClamp: 2, WebkitBoxOrient: "vertical", overflow: "hidden" }}>{property.title}</h3>
        <div style={{ display: "flex", alignItems: "center", gap: "5px", marginBottom: "14px" }}>
          <svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="#10C4C3" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round">
        ...
        <div style={{ display: "flex", borderTop: "1px solid rgba(245,242,236,0.07)", paddingTop: "14px" }}>
          {[{ value: property.bedrooms, label: COMMERCIAL_CATEGORIES.includes(property.type) ? "Rooms" : "Beds" }, { value: property.bathrooms, label: COMMERCIAL_CATEGORIES.includes(property.type) ? "Wash" : "Baths" }, { value: property.area_sqft?.toLocaleString("en-IN"), label: "sqft" }].map((s, i) => s.value != null && (
            <div key={i} style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", gap: "3px", borderRight: i < 2 ? "1px solid rgba(245,242,236,0.07)" : "none" }}>
              <span style={{ fontSize: "13px", fontWeight: 600, color: "#020C1C" }}>{s.value}</span>
...
function FilterCheck({ label, checked, onChange }: { label: string; checked: boolean; onChange: () => void }) {
  return (
    <label style={{ display: "flex", alignItems: "center", gap: "10px", cursor: "pointer", marginBottom: "10px" }}>
      <div onClick={onChange} style={{ width: "16px", height: "16px", borderRadius: "4px", flexShrink: 0, border: checked ? "none" : "1.5px solid rgba(245,242,236,0.25)", background: checked ? "#10C4C3" : "transparent", display: "flex", alignItems: "center", justifyContent: "center", transition: "all 0.15s" }}>
        {checked && <svg width="10" height="8" viewBox="0 0 10 8" fill="none"><path d="M1 4l2.5 2.5L9 1" stroke="#020C1C" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round" /></svg>}
      </div>
      <span onClick={onChange} style={{ fontSize: "13px", color: checked ? "#020C1C" : "rgba(245,242,236,0.55)", transition: "color 0.15s" }}>{label}</span>
    </label>
  );
}
...
export default function PropertiesPage() {
  ...
  return (
    <>
      ...
      <div style={{ minHeight: "100vh", background: "#020C1C", color: "#020C1C" }}>
        ...
        {/* Hero */}
        <div className="pr-hero" style={{ position: "relative", zIndex: 1, marginTop: "64px", background: "linear-gradient(135deg, #081c12 0%, #020C1C 60%, #132e1f 100%)", borderBottom: "1px solid rgba(201,168,76,0.12)", padding: "48px 48px 40px" }}>
          <div style={{ maxWidth: "1400px", margin: "0 auto" }}>
            <div style={{ display: "flex", alignItems: "center", gap: "8px", marginBottom: "16px" }}>
              ...
              <span style={{ fontSize: "12px", color: "#10C4C3" }}>Properties</span>
            </div>
            <h1 style={{ fontFamily: "'Cal Sans', Georgia, serif", fontSize: "clamp(36px, 5vw, 56px)", fontWeight: 300, color: "#020C1C", lineHeight: 1.1, marginBottom: "10px" }}>
              Find Your <em style={{ fontStyle: "italic", color: "#10C4C3" }}>Perfect Home</em>
            </h1>
        ...
        <div className="pr-layout" style={{ position: "relative", zIndex: 1, maxWidth: "1400px", margin: "0 auto", padding: "32px 48px 80px", display: "flex", gap: "28px", alignItems: "flex-start" }}>
          ...
          <aside className="pr-sidebar" style={{ width: "280px", flexShrink: 0, position: "sticky", top: "72px", background: "rgba(245,242,236,0.03)", border: "1px solid rgba(245,242,236,0.08)", borderRadius: "16px", overflow: "hidden", maxHeight: "calc(100vh - 108px)", overflowY: "auto" }}>
            <div style={{ padding: "20px 20px 16px", borderBottom: "1px solid rgba(245,242,236,0.07)", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
              ...
              {hasFilters && <button onClick={clearFilters} style={{ fontSize: "11px", fontWeight: 600, letterSpacing: "0.08em", textTransform: "uppercase", color: "#10C4C3", background: "transparent", border: "none", cursor: "pointer", fontFamily: "'Cal Sans', sans-serif" }}>Clear All</button>}
            </div>
            ...
            <div style={{ padding: "20px" }}>
              ...
              <div style={{ marginBottom: "24px" }}>
                ...
                <div style={{ display: "flex", gap: "6px" }}>
                  {(["all", "sale", "rent"] as const).map(t => (
                    <button key={t} onClick={() => setListingType(t)} style={{ flex: 1, padding: "8px 4px", borderRadius: "8px", fontSize: "12px", fontWeight: 600, textTransform: "capitalize", background: listingType === t ? "#10C4C3" : "rgba(245,242,236,0.05)", border: listingType === t ? "none" : "1px solid rgba(245,242,236,0.1)", color: listingType === t ? "#020C1C" : "rgba(245,242,236,0.55)", cursor: "pointer", fontFamily: "'Cal Sans', sans-serif", transition: "all 0.15s" }}>
              ...
              <div style={{ marginBottom: "24px" }}>
                ...
                <div style={{ position: "relative" }}>
                  <select value={city} onChange={e => setCity(e.target.value)} style={{ width: "100%", padding: "10px 36px 10px 14px", background: "rgba(245,242,236,0.05)", border: "1px solid rgba(245,242,236,0.12)", borderRadius: "8px", color: city === "all" ? "rgba(245,242,236,0.45)" : "#020C1C", fontSize: "13px", fontFamily: "'Cal Sans', sans-serif", cursor: "pointer" }}>
              ...
              <div style={{ marginBottom: "24px" }}>
                ...
                <div style={{ display: "flex", gap: "8px" }}>
                  {[{ placeholder: "Min", value: minPrice, onChange: setMinPrice }, { placeholder: "Max", value: maxPrice, onChange: setMaxPrice }].map(({ placeholder, value, onChange }) => (
                    <input key={placeholder} type="number" placeholder={placeholder} value={value} onChange={e => onChange(e.target.value)} style={{ flex: 1, padding: "9px 10px", background: "rgba(245,242,236,0.05)", border: "1px solid rgba(245,242,236,0.12)", borderRadius: "8px", color: "#020C1C", fontSize: "12px", fontFamily: "'Cal Sans', sans-serif", outline: "none" }} />
                  ))}
                </div>
                <div style={{ display: "flex", gap: "6px", marginTop: "8px", flexWrap: "wrap" }}>
                  {[{ label: "< 50L", min: "", max: "5000000" }, { label: "1–3 Cr", min: "10000000", max: "30000000" }, { label: "3 Cr+", min: "30000000", max: "" }].map(p => (
                    <button key={p.label} onClick={() => { setMinPrice(p.min); setMaxPrice(p.max); }} style={{ padding: "4px 10px", borderRadius: "100px", fontSize: "11px", fontWeight: 500, background: "rgba(201,168,76,0.08)", border: "1px solid rgba(201,168,76,0.2)", color: "#10C4C3", cursor: "pointer", fontFamily: "'Cal Sans', sans-serif" }}>{p.label}</button>
                  ))}
                </div>
              </div>
          ...
          <div style={{ flex: 1, minWidth: 0 }}>
            ...
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: "24px", gap: "16px", flexWrap: "wrap" }}>
              <div>
                {loading ? (
                  ...
                  <span style={{ fontSize: "14px", color: "rgba(245,242,236,0.55)" }}>
                    <span style={{ fontFamily: "'Cal Sans', Georgia, serif", fontSize: "22px", fontWeight: 600, color: "#020C1C", marginRight: "6px" }}>{filtered.length}</span>
              ...
              <div style={{ display: "flex", alignItems: "center", gap: "12px" }}>
                <div style={{ position: "relative" }}>
                  <select value={sortBy} onChange={e => setSortBy(e.target.value)} style={{ padding: "9px 36px 9px 14px", background: "rgba(245,242,236,0.05)", border: "1px solid rgba(245,242,236,0.12)", borderRadius: "8px", color: "#020C1C", fontSize: "13px", fontFamily: "'Cal Sans', sans-serif", cursor: "pointer" }}>
                    <option value="featured" style={{ background: "#020C1C" }}>Featured First</option>
                    <option value="price_asc" style={{ background: "#020C1C" }}>Price: Low to High</option>
                    <option value="price_desc" style={{ background: "#020C1C" }}>Price: High to Low</option>
                  </select>
                  <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="rgba(245,242,236,0.4)" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" style={{ position: "absolute", right: "12px", top: "50%", transform: "translateY(-50%)", pointerEvents: "none" }}><polyline points="6 9 12 15 18 9" /></svg>
                </div>
                <div style={{ display: "flex", background: "rgba(245,242,236,0.05)", border: "1px solid rgba(245,242,236,0.1)", borderRadius: "8px", overflow: "hidden" }}>
                  {(["grid", "list"] as const).map(mode => (
                    <button key={mode} onClick={() => setViewMode(mode)} style={{ width: "36px", height: "36px", display: "flex", alignItems: "center", justifyContent: "center", background: viewMode === mode ? "rgba(201,168,76,0.15)" : "transparent", border: "none", cursor: "pointer", borderRight: mode === "grid" ? "1px solid rgba(245,242,236,0.1)" : "none" }}>
                      {mode === "grid" ? (
                        <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke={viewMode === "grid" ? "#10C4C3" : "rgba(245,242,236,0.4)"} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><rect x="3" y="3" width="7" height="7" /><rect x="14" y="3" width="7" height="7" /><rect x="3" y="14" width="7" height="7" /><rect x="14" y="14" width="7" height="7" /></svg>
                      ) : (
                        <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke={viewMode === "list" ? "#10C4C3" : "rgba(245,242,236,0.4)"} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><line x1="3" y1="6" x2="21" y2="6" /><line x1="3" y1="12" x2="21" y2="12" /><line x1="3" y1="18" x2="21" y2="18" /></svg>
                      )}
                    </button>
                  ))}
                </div>
            ...
            {loading ? (
              ...
              <div style={{ display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: "80px 24px", textAlign: "center", background: "rgba(245,242,236,0.02)", border: "1px solid rgba(245,242,236,0.06)", borderRadius: "16px" }}>
                ...
                <h3 style={{ fontFamily: "'Cal Sans', Georgia, serif", fontSize: "28px", fontWeight: 400, color: "#020C1C", marginBottom: "12px" }}>No Properties Found</h3>
                ...
                <button onClick={clearFilters} style={{ padding: "11px 28px", background: "#10C4C3", border: "none", borderRadius: "8px", color: "#020C1C", fontSize: "13px", fontWeight: 700, letterSpacing: "0.08em", textTransform: "uppercase", cursor: "pointer", fontFamily: "'Cal Sans', sans-serif" }}>Clear Filters</button>
              </div>
            ) : viewMode === "grid" ? (
                ...
                {paginatedItems.map(p => (
                  ...
                  >
                    <div className="pr-list-img" style={{ width: "280px", flexShrink: 0, position: "relative" }}>
                      ...
                      <span style={{ position: "absolute", top: "12px", left: "12px", padding: "4px 10px", borderRadius: "100px", fontSize: "10px", fontWeight: 700, letterSpacing: "0.1em", textTransform: "uppercase", background: p.listing_type === "rent" ? "rgba(45,106,79,0.9)" : "rgba(201,168,76,0.9)", color: p.listing_type === "rent" ? "#0A1526" : "#020C1C", backdropFilter: "blur(8px)" }}>
                    ...
                    <div style={{ flex: 1, padding: "24px 28px", display: "flex", flexDirection: "column", justifyContent: "space-between" }}>
                      <div>
                        <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", gap: "16px", marginBottom: "8px" }}>
                          <h3 style={{ fontFamily: "'Cal Sans', Georgia, serif", fontSize: "20px", fontWeight: 600, color: "#020C1C", lineHeight: 1.3 }}>{p.title}</h3>
                          <span style={{ fontFamily: "'Cal Sans', Georgia, serif", fontSize: "22px", fontWeight: 600, color: "#10C4C3", whiteSpace: "nowrap" }}>{formatPrice(p.price, p.listing_type)}</span>
                        </div>
                        <div style={{ display: "flex", alignItems: "center", gap: "5px", marginBottom: "12px" }}>
                          <svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="#10C4C3" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><path d="M21 10c0 7-9 13-9 13s-9-6-9-13a9 9 0 0 1 18 0z" /><circle cx="12" cy="10" r="3" /></svg>
                      ...
                      <div style={{ display: "flex", gap: "20px", marginTop: "16px", paddingTop: "16px", borderTop: "1px solid rgba(245,242,236,0.07)" }}>
                        {[{ label: COMMERCIAL_CATEGORIES.includes(p.type) ? "Rooms" : "Beds", value: p.bedrooms }, { label: COMMERCIAL_CATEGORIES.includes(p.type) ? "Wash" : "Baths", value: p.bathrooms }, { label: "sqft", value: p.area_sqft?.toLocaleString("en-IN") }].map(s => s.value != null && (
                          <div key={s.label} style={{ display: "flex", flexDirection: "column", gap: "2px" }}>
                            <span style={{ fontSize: "15px", fontWeight: 600, color: "#020C1C" }}>{s.value}</span>
                            <span style={{ fontSize: "10px", color: "rgba(245,242,236,0.35)", textTransform: "uppercase", letterSpacing: "0.06em" }}>{s.label}</span>
                          </div>
                        ))}
                        {p.is_featured && <span style={{ marginLeft: "auto", padding: "4px 12px", borderRadius: "100px", alignSelf: "center", fontSize: "10px", fontWeight: 700, letterSpacing: "0.1em", textTransform: "uppercase", color: "#10C4C3", border: "1px solid rgba(201,168,76,0.3)" }}>Premium</span>}
                      </div>
                    </div>
                  </a>
                ))}
